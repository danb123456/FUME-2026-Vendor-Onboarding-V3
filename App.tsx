...
const App: React.FC = () => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      console.error(err);
      alert("Submission failed. Progress is saved locally.");
    } finally {
    ...
    return (
      <div className="flex items-center justify-center min-h-screen bg-gray-50 p-6 text-center">
        <div className="bg-white p-10 rounded-3xl shadow-xl max-w-lg">
          ...
          <h2 className="text-3xl font-black mb-4 uppercase italic">ONBOARDED</h2>
          <p className="text-gray-600 mb-8 font-medium">Record synced for Vendor ID: <span className="text-orange-600 font-bold">{formData.vendorId}</span>. Previous records were overwritten.</p>
          <button onClick={() => window.location.reload()} className="bg-orange-600 text-white font-black px-8 py-4 rounded-xl uppercase tracking-widest">Restart</button>
        </div>
      </div>
    );
  }
  ...
  return (
    <div className="max-w-4xl mx-auto py-12 px-4">
      <div className="fixed bottom-6 right-6 z-50">
        ...
        >
          {isDraftSaved ? '✓ Saved' : 'Save Progress'}
        </button>
      </div>
      ...
      <form onSubmit={handleSubmit} className="space-y-10">
        ...
        <div className="bg-white rounded-3xl shadow-sm border border-gray-100 overflow-hidden">
          ...
          <div className="p-8 grid grid-cols-1 md:grid-cols-2 gap-6">
            <FormField label="Vendor ID" required helper="Matches existing spreadsheet row" value={formData.vendorId} onChange={(v) => setFormData({...formData, vendorId: v})} />
            ...
            <div>
              <label className="block text-sm font-black text-gray-700 mb-2 uppercase">State Fayre? *</label>
              <select 
        ...
        <div className="bg-white rounded-3xl shadow-sm border border-gray-100 overflow-hidden">
          ...
          <div className="p-8 space-y-8">
            <div className="bg-orange-50 p-5 rounded-2xl border border-orange-200">
              <p className="text-sm text-orange-950 font-bold uppercase tracking-tight">
                2.3m L/R & 2m Behind included. Excess @ £100/m.
              </p>
            </div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
              {['spaceLeft', 'spaceRight', 'spaceBehind'].map((f) => (
                <div key={f}>
                  <label className="block text-[10px] font-black text-gray-400 uppercase mb-1">{f.replace('space', 'Space ')}</label>
                  <select className="w-full px-3 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-orange-500 outline-none" value={(formData as any)[f]} onChange={(e) => setFormData({...formData, [f]: e.target.value})}>
            ...
            <div>
              ...
              <div className="grid grid-cols-2 sm:grid-cols-3 gap-3">
                {REASON_OPTIONS.map((opt) => (
                  <button key={opt} type="button" onClick={() => setFormData({...formData, externalSpaceReason: opt as ExternalSpaceReason})} className={`px-4 py-3 rounded-xl border-2 font-black text-xs uppercase ${formData.externalSpaceReason === opt ? 'border-orange-600 bg-orange-600 text-white shadow-lg' : 'border-gray-200 bg-white text-gray-500 hover:border-gray-300'}`}>
                    {opt}
                  </button>
                ))}
              </div>
          ...
          <div className="p-8 space-y-6">
            ...
            <div className="pt-6 border-t border-gray-100">
              <div className="flex justify-between items-center mb-6">
                <h3 className="font-black text-gray-800 uppercase italic">Equipment Inventory</h3>
                <button type="button" onClick={addEquipment} className="bg-orange-600 text-white text-[10px] font-black px-5 py-2 rounded-full uppercase tracking-widest">+ Add Item</button>
              </div>
              <div className="space-y-3">
                {formData.equipment.map((item, index) => (
                  <div key={item.id} className="flex gap-4 items-end bg-gray-50 p-4 rounded-2xl border border-gray-200">
                    <div className="flex-1">
                      <select className="w-full px-3 py-2 border border-gray-300 rounded-lg bg-white font-bold text-sm" value={item.type} onChange={(e) => {
                    ...
                    <div className="flex-1">
                      <select className="w-full px-3 py-2 border border-gray-300 rounded-lg bg-white font-bold text-sm" value={item.socket} onChange={(e) => {
          ...
          <div className="p-8 space-y-8">
            <div className={`p-4 rounded-xl border-l-4 shadow-sm ${formData.comingToStateFayre === 'Yes' ? 'bg-purple-50 border-purple-600 text-purple-900' : 'bg-blue-50 border-blue-500 text-blue-800'}`}>
              <p className="text-xs font-black uppercase tracking-widest">
                {formData.comingToStateFayre === 'Yes' ? 'State Fayre: Valid until July 1st 2026.' : 'FUME: Valid until June 14th 2026.'}
              </p>
            </div>
              ...
              {PAPERWORK_ITEMS.map((item) => (
                <div key={item.id} className="space-y-2">
                  ...
                  <div className="flex flex-col">
                    <label className="text-[10px] uppercase font-black text-gray-400 mb-1">Expiry Date</label>
            ...
            <div className="pt-6 border-t border-gray-100">
              <label className="block text-sm font-black text-gray-700 mb-2 uppercase">Finished Paperwork?</label>
              <select className="w-full px-4 py-2 border border-gray-300 rounded-lg" value={formData.paperworkStatus} onChange={(e) => setFormData({...formData, paperworkStatus: e.target.value as any})} required>
                <option value="">Select...</option>
                <option value="Yes">Yes</option>
            ...
            {[
              ...
              <div key={d.key} className="grid grid-cols-1 md:grid-cols-3 gap-6 p-6 bg-gray-50 rounded-2xl border border-gray-200">
                <div className="flex flex-col justify-center">
                  ...
                  <div className="text-[10px] font-black text-gray-400 uppercase tracking-widest">{d.label} Dish</div>
                </div>
        ...
        <div className="bg-white rounded-3xl shadow-sm border border-gray-100 overflow-hidden">
          ...
          <div className="p-8 space-y-6">
            <div className="bg-gray-900 text-white p-5 rounded-2xl uppercase font-black tracking-widest text-[10px] text-center">
              Min 6 staff required. Non-compliance results in fines.
            </div>
            <div className="flex justify-between items-center">
              <h3 className="font-black text-gray-800 uppercase italic">Staff ({formData.staff.length}/6)</h3>
              <button type="button" onClick={addStaff} className="bg-black text-white text-[10px] font-black px-6 py-2.5 rounded-full uppercase transition-transform hover:scale-105">+ Add Staff</button>
            </div>
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
              {formData.staff.map((s, index) => (
                <div key={s.id} className="flex gap-4 items-center bg-gray-50 p-4 rounded-2xl border border-gray-200 font-black uppercase text-[10px]">
                  <span className="text-gray-400">#{index+1}</span>
                  <select className="flex-1 bg-transparent tracking-tighter" value={s.role} onChange={(e) => {
        ...
        <div className="bg-white rounded-3xl shadow-sm border border-gray-100 overflow-hidden">
          ...
          <div className="p-8 grid grid-cols-1 md:grid-cols-2 gap-6">
            <FormField label="Vehicle Reg" value={formData.vehicleReg} onChange={(v) => setFormData({...formData, vehicleReg: v})} required />
            <FormField label="Instagram Handle" value={formData.instagram} onChange={(v) => setFormData({...formData, instagram: v})} />
            <div className="md:col-span-2">
              <label className="block text-sm font-black text-gray-700 mb-2 uppercase">Questions/Comments</label>
              <textarea rows={3} className="w-full px-4 py-3 border border-gray-300 rounded-2xl" value={formData.comments} onChange={(e) => setFormData({...formData, comments: e.target.value})} />
            </div>
          </div>
        </div>
        ...
        <div className="space-y-4 pt-6">
          <button 
            ...
            className={`w-full py-8 rounded-[40px] text-3xl font-black text-white shadow-2xl transition-all uppercase italic tracking-tighter ${
              isSubmitting || formData.staff.length < MIN_STAFF_COUNT ? 'bg-gray-300' : 'bg-orange-600 hover:bg-orange-700 active:scale-[0.98]'
            }`}
          >
            {isSubmitting ? 'UPLOADING...' : 'SUBMIT ONBOARDING'}
          </button>
        </div>
      </form>

      <footer className="mt-24 py-12 border-t border-gray-200 text-center">
        <p className="text-gray-400 text-[9px] font-black uppercase tracking-[0.4em] italic">&copy; 2026 FUME Festival | master-onboard-v1</p>
      </footer>
    </div>
  );
};
